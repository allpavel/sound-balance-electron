...
describe("tracksRepository", () => {
	...
	describe("addMany", () => {
		// NOTE: `allKeys` defaults to `true`. The default path returns an array of
		// primary keys. Passing `{ allKeys: false }` would make `bulkAdd` return a
		// single key that gets spread char-by-char into `resultIds` — that branch
		// is intentionally not exercised here as it is not used by the app.
		...
		it("matches existing tracks by filePath, not by id", async () => {
			...
			// expect(rows[0].collectionIds).toEqual(["all", "mix1"]);
		});

		it("returns an empty array for an empty input and writes nothing", async () => {
			expect(
				await tracksRepository.addMany([], { targetCollectionId: "all" }),
			).toEqual([]);
	...
	describe("getSelectedTracks", () => {
		...
		it("excludes tracks where selected is unset", async () => {
			await db.tracks.add(makeTrack({ id: "t1", selected: undefined }));
